...
def import_teamlists_data(excel_file_path):
    ...
    # Clean column Names: "Player Number" -> "Player_Number", "Player Name" -> "Player_Name"
    ...
    with conn.cursor() as cur:
        for idx, row in df.iterrows():
            # Prepare the row values (handle NaN as None)
            row_values = [None if pd.isna(val) else val for val in row]
            # Build the insert query based on the column Names
